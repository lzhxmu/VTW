...
class LlavaMetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        ### VTW ################################################################################################
        from .language_model.VTW import temp_cache
        # ====================================================================================================
        ...
        if (image_tower is None and video_tower is None) or images is None or input_ids.shape[1] == 1:
            if past_key_values is not None and (image_tower is not None or video_tower is not None) and images is not None and input_ids.shape[1] == 1:
                if temp_cache.use_vtw:
                    target_shape = past_key_values[-1][-1].shape[-2] + 1 + temp_cache.image_token_nums
                else:
                    target_shape = past_key_values[-1][-1].shape[-2] + 1

                    if temp_cache.mode == 'fastv':
                        target_shape += int(temp_cache.image_token_nums * 0.5)
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            cur_new_labels = []
            ### VTW ################################################################################################
            temp_cache.system_token_nums = cur_input_embeds_no_im[0].shape[0]
            temp_cache.image_token_nums = num_images * 257
